...
- **購入リンクの保存が効かなかったときに黙って成功を返さなくなった。** `update_post_meta()` は書き込みに失敗しても false を返すだけなので、従来は保存されていないのに REST が 201/200 を返していた。書き込み後に読み直して確かめ、入っていなければ **HTTP 500 と `affilicard_save_failed`** を返す（`/bulk` では該当項目だけを `error` にし、同じコードを添える）。ロック競合の **409 とは別物**——409 はやり直せば通るが、500 は原因（別プラグインの meta フィルタ、壊れた meta 行）を取り除くまで直らない
- **同じ商品への同時保存を商品単位のロックで直列化した。** 保存が競合してロックを取れなかった場合、古い値で上書きせずに保存を断る。REST（`affilicard/v1/products` の作成・更新）は **HTTP 409 と `affilicard_listing_locked`** を返し、`/bulk` では該当項目だけを `error` にする（他の項目は通る）。自動作成は一時的失敗として扱い、ジョブを積み直す。ロックの待ち時間は 10 秒なので、これが起きるのは同一商品に実際の競合があるときだけ
