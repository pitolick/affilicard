...
- **購入リンクの保存が効かなかったときに黙って成功を返さなくなった。** `update_post_meta()` は書き込みに失敗しても false を返すだけなので、従来は保存されていないのに REST が 201/200 を返していた。書き込み後に読み直して確かめ、入っていなければ **HTTP 500 と `affilicard_save_failed`** を返す（`/bulk` では該当項目だけを `error` にし、同じコードを添える）。ロック競合の **409 とは別物**——409 はやり直せば通るが、500 は原因（別プラグインの meta フィルタ、壊れた meta 行）を取り除くまで直らない。**エラー本文には対象商品の `id` と `unsaved_fields` が入る**（下記）
...
- **保存を断った／失敗した REST 応答が、対象商品の `id` と `unsaved_fields` を返すようになった**（`affilicard_listing_locked` の 409・`affilicard_save_failed` の 500、および `/bulk` の該当項目）。
  ...
  **`unsaved_fields`（保存されなかったフィールド名の配列。現状は常に `["listings"]`）は、その商品が「どこまで保存されているか」を表す。** 保存は投稿行 → 購入リンク以外のメタ → `listings` の順に進み、`listings` で失敗したときだけこれらのエラーになるため、**409 / 500 が返る時点で要求の大半は保存済み**である。これが分からないと、呼び出し側は全項目を送り直す（＝同時に入った別経路の編集を巻き戻す）か、何も直さない（＝購入リンクの無い商品を放置する）かの二択になる。載らなかったフィールドは保存されている、と読んでよい。**`unsaved_fields` を持たない 500 は「1 つも保存されていない」**（投稿行を作れなかった／更新できなかったとき）で、`id` の有無と同じく有無そのものが signal になる
