...
- **移行そのものの書き込み**で、`regular_url` を持たず `affiliate_url` のみを持つ listing（手入力で温存されていたデータ）が消えていた不具合を修正した。移行はこの形の購入リンクも温存し、件数を運用向けに可視化する（管理画面の通知）。
  **既知の制限**: この温存は移行の書き込み 1 回分に限った例外である。`ProductSchema::sanitizeOffers()` は通常の保存では `regular_url` が空の offer を常に弾くため、温存された listing であっても、この商品を次に保存する（**同じ商品の別プラットフォームの価格更新による自動保存も含む**）と、通常 URL を追加していない限りこの購入リンクは自動的に削除される。管理画面の通知が示す件数分の listing には、消える前に通常 URL を追加すること
