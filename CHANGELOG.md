...
- **保存が効かなかったときに黙って成功を返さなくなった。** `update_post_meta()` の戻り値は「入ったかどうか」を表さない（失敗しても false を返すだけだが、ほかのプラグインが `update_post_metadata` フィルタで短絡すると **1 バイトも書かずに成功を返す**）ため、従来は保存されていないのに REST が 201/200 を返していた。**書き込み後に読み直して確かめ**、入っていなければ **HTTP 500 と `affilicard_save_failed`** を返す（`/bulk` では該当項目だけを `error` にし、同じコードを添える）。確かめる対象は**要求が実際に運んでくるメタ**——`listings` に加えて `product_type` / `stock_status` / `extras` / `release_date` である。ロック競合の **409 とは別物**——409 はやり直せば通るが、500 は原因（別プラグインの meta フィルタ、壊れた meta 行）を取り除くまで直らない。**エラー本文には対象商品の `id` と `unsaved_fields` が入る**（下記）
- **同じ商品への同時保存を商品単位のロックで直列化した。** 保存が競合してロックを取れなかった場合、古い値で上書きせずに保存を断る。REST（`affilicard/v1/products` の作成・更新）は **HTTP 409 と `affilicard_listing_locked`** を返し、`/bulk` では該当項目だけを `error` にする（他の項目は通る）。自動作成は一時的失敗として扱い、ジョブを積み直す。ロックの待ち時間は 10 秒なので、これが起きるのは同一商品に実際の競合があるときだけ。**エラー本文には対象商品の `id` と `unsaved_fields` が入る**（下記）
- **保存を断った／失敗した REST 応答が、対象商品の `id` と `unsaved_fields` を返すようになった**（`affilicard_listing_locked` の 409・`affilicard_save_failed` の 500、および `/bulk` の該当項目）。
  **作成（`POST /affilicard/v1/products`）でこれが起きた時点で商品は既に作られている**——投稿行を作ってから購入リンクを書くため——にもかかわらず、従来の応答は ID を持たなかった。呼び出し側は商品ができたことを知る術がなく、再試行が POST のやり直しになって**同じ商品が増え続けた**。`id` があれば、やり直しを `PATCH /affilicard/v1/products/{id}` に変えられる。
  規則は「サーバが実在を知っている商品の ID だけを載せる」の 1 つで、作成・更新で変えない。**唯一 `id` を持たないのは、作成で投稿行そのものを作れなかった 500** で、このときは商品が 1 件も無い。**`id` の有無がそのまま「`PATCH` で直せるか／`POST` し直すか」を表す**。
  **`unsaved_fields`（入らなかったと分かっているフィールド名の配列。値は `product_type` / `stock_status` / `extras` / `release_date` / `listings` のいずれか）は、やり直しで送り直すべきものを名指しする。** 保存は投稿行 → 購入リンク以外のメタ → `listings` の順に進み、**書いたメタは読み直して確かめる**ため、**409 / 500 が返る時点で要求の大半は保存済み**である。これが分からないと、呼び出し側は全項目を送り直す（＝同時に入った別経路の編集を巻き戻す）か、何も直さない（＝購入リンクの無い商品を放置する）かの二択になる。
  **ただし「ここに載らない＝保存された」とは読めない。** 読み直して確かめているのは要求が実際に運んでくるメタだけで、`mask_blur` / `mask_r18` / `mask_label` / `schema_version` と投稿行の `title` / `content` / `status` は確かめていない（前者はこの API から設定できず、書かれるのは既存値か既定値なので潰されても要求は失われない。後者は投稿 API の戻り値しか見ていない）。この配列は**確実に送り直すべきものの一覧**であって、保存された範囲の裏返しではない。**`unsaved_fields` を持たない 500 は「1 つも保存されていない」**（投稿行を作れなかった／更新できなかったとき）で、`id` の有無と同じく有無そのものが signal になる
...
- **アップグレードしても offers 移行が 1 度も始まらなかった不具合を修正した。** 移行の投入は `plugins_loaded` で行っていたが、Action Scheduler のデータストアが初期化されるのは `init`（優先度 1）である。それより前に `as_schedule_single_action()` を呼ぶと**何も積まずに 0 が返る**ため、移行アクションは 1 件も作られず、未完の印（カーソル）だけが残って毎リクエスト同じ空振りを繰り返していた。その間、商品は旧形式のまま残り、通常 URL も外部 ID も持たない購入リンクの温存・管理画面への通知も一切起こらなかった。投入を `action_scheduler_init` まで遅らせて修正した。あわせて、同じ呼び出しが `_doing_it_wrong()` を鳴らし、`WP_DEBUG` 有効のサイトで「Cannot modify header information — headers already sent」を出して管理画面を壊していた問題も解消した
- **移行がまだ到達していない listing への価格更新を、移行が終わるまで見送るようにした。** 価格更新の保存は listing を `offers[]` へ揃えてから書き戻すため、v3 の flat な listing へ価格更新が先に届くと、その保存が flat → `offers[]` の変換を兼ねてしまう。その変換は通常の sanitize を通るので、**移行だけが温存する「通常 URL も外部 ID も持たない購入リンク」がそこで消え、温存件数も増えず管理画面の通知も出なかった**（移行があとから到達しても、その listing は既に `offers[]` を持つため数える機会そのものが失われる）。現在は「移行が未完」かつ「その listing が未変換」のあいだだけ、価格更新を書き込まずに見送る。**これは予防的な措置である**——本番カタログ 1,500 件の実測時点で該当データは 0 件で、失われた実害は確認されていないが、失われても気づく手段が無い種類の損失であるため塞いだ。
  見送った更新は失われない: 書き込まない＝最終取得日時が据え置かれるため、掃引の再取得判定が引き続き成立し、次の周回で同じ listing がまた積まれる（移行が完走すれば見送りは効かなくなり、そのまま通常どおり取得される）。見送りの範囲は狭く、**移行中でも既に変換済みの listing は通常どおり更新される**
...
- **棚卸し**: 最後に記事へ掲載した日から一定期間が過ぎた商品の自動更新を停止する。既定は 180 日で、一般設定から有効/無効と期間を変更できる。記事を更新すれば対象から戻る
