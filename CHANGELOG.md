...
- 一般設定の読み取り（`GET /affilicard/v1/settings`）は、`manage_options` を持たない呼び出し元（商品編集画面のための `edit_posts` 許可）に対して `fallback_on_terminal` のみを返すようにした。従来は一般設定オブジェクト全体（キューの状態・保持期間・スロットル上書きなど。認証情報は含まない）を返していた。URL と許可 capability は変更していない
