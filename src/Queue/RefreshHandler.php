...
final class RefreshHandler extends ThrottledActionHandler {
	...
	protected function performWork( array $args ): bool {
		$ok = $this->refresher->refreshOne( (int) $args['post_id'], (string) $args['platform'] );
		if ( $ok ) {
			// B: fetch 成功で give-up マーカーを消す（外部 ID が復旧した listing を通常周期に戻す）。
	...
	/**
	 * B: terminal failure（MAX_ATTEMPTS 到達＝廃盤/無効 ID で恒久的に解決できない）時に
	 * give-up マーカーを永続化する。QueueMaintenance::sweep() が GIVEUP_COOLDOWN の間この
	 * listing をスキップし、再取得 TTL 毎の毎周回リトライ（API 浪費・completed チャーン）を抑える。
	 *
