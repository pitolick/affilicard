...
- 同一 listing 内で識別子が重複する offer は**後勝ちでマージ**する（同じ SKU が 2 つ並ぶ状態を作らない）。**値だけでなく配列上の位置も後の出現に合わせる**——値だけ差し替えて最初の位置に残すと、`display_order` が同値のときの並び（§3-5 の「同値は配列の出現順」）が入力順とずれる
...
**現時点では 0 件または 1 件しか返さない**が、返り値は配列とする。将来「同一 platform の複数の購入リンクを同時に見せる」（出品者違いの並列表示、通常価格の打ち消し表示）に進む場合、**この関数が返す件数を増やすだけ**で描画も更新も追随する。描画と更新が別々に改修対象になることを避ける。
...
- 移行後の `offers` は、**取得結果フィールド（`external_id` / `regular_url` / `affiliate_url` / `price` / `image_url` / `search_key`）を 1 つでも持つ listing なら 1 件**である。設定（`platform` / `enabled` 等）しか持たない listing は **0 件**にする——空の offer を作ると、移行前は購入リンクを持たなかった listing が移行後は 1 件持つことになり、存在しない購入リンクを選択係が選び、レート制限の枠取りも価格更新もそれを対象にしてしまう。判定は読み取り側のフォールバックと同じ `LegacyOffer::hasFlatFetchFields()` を使い、移行前後で `LegacyOffer::offersWithFallback()` の件数が変わらないようにする
- 選択係は offer が 1 件ならそれを返し、0 件なら何も返さない（移行前に flat な取得結果が無かった listing とまったく同じ）。したがって**挙動は移行前と完全に同一**である
