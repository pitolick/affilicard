...
2. **`accessKey` が必須**。クエリでも受理されるが、**HTTP ヘッダー `accessKey` で送る**（サーバーサイドでのクエリ／アクセスログ露出を避けるため。実測で `accessKey` ヘッダー＋Origin → 200 を確認）。`applicationId` 単独では 400。
...
> **`accessKey` の秘匿度について**: `pk_` プレフィックスの publishable 系キーで、クライアントサイド利用も想定し `Origin`/`Referer` 許可リストで縛る半公開キー。OAuth シークレットほど厳格な秘匿は不要だが、サーバーサイドではログ露出を避けるためヘッダー送信を採る。affilicard 側では `ProviderCredentials` が AES 暗号化して WP オプション保存するため at-rest も保護される。`applicationId` は URL に載る半公開でよい。
...
- **共通 HttpClient の抽出**（`DmmProvider` と `RakutenProvider` の `wp_remote_get` 定型を DRY 化する refactor）。**rule of three で保留**＝Provider 2 つでは重複を許容し、3 つ目＝Amazon（Creators API は OAuth トークン取得＋POST 系で GET と異質）まで待って、共通パターンが見えてから抽象化を判断する。`DmmProvider` には本 PR で手を入れない。
...
- **必須クエリ**: `applicationId` ＋ 検索キー1つ以上（`keyword` / `title` / `author` / `publisherName` / `itemNumber` / `koboGenreId`）
- **任意クエリ**: `affiliateId`（付与で `affiliateUrl` 生成）/ `format=json` / `formatVersion=2` / `hits`
- **必須ヘッダー**: **`accessKey`（アクセスキー）** ＋ **`Origin`（許可ドメイン）**。`Referer` も併せて送る。`accessKey` はクエリでも受理されるがログ露出回避のためヘッダー送信を採用。
...
### 4-3. リクエストヘッダーと Origin/Referer ドメインの解決

- リクエストは **ヘッダー: `accessKey`（アクセスキー）＋`Origin`＋`Referer`** ／ **クエリ: `applicationId`・`affiliateId`・検索キー・`format`/`formatVersion`/`hits`** で組み立てる。
- Origin/Referer に使うドメインの解決:
...
- 中立キーワードで `hits=1` 検索し、`accessKey`/`Origin`/`Referer` をヘッダー付与して呼ぶ。
...
- `fetch()`:
  ...
  - **`accessKey`/`Origin`/`Referer` ヘッダー送出**（`accessKey` はクエリに載らずヘッダーで送ること／`allowed_domain` 空 → home_url()・指定 → その値。`wp_remote_get` の args を検証）。
