...
> **日時のタイムゾーンについて**: Scheduled Actions／「更新キュー（ジョブ一覧）」に表示される日時は WordPress 設定 → 一般の「タイムゾーン」に従います。`+0000`（UTC）表示になる場合は、サイトのタイムゾーンが未設定（`timezone_string` が空・`gmt_offset=0`）です。設定 → 一般でタイムゾーンを設定してください（affilicard は Action Scheduler の日時描画を制御しません）。
